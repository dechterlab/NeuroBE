...
int32_t main(int32_t argc, char* argv[])
{
	...
	for (int32_t i = 0 ; i < nArgs; i++) {
		...
		if (0 == stricmp("-s", sArgID.c_str()))
        ...
        else if (0 == stricmp("-batch_size",sArgID.c_str() )) 		//Hyperparameters for NeuroBE from here
            ...
            global_config.var_dim = atof(sArg.c_str());
		}
    ...
    std::string o_file2 = global_config.out_file + ".txt";
    ...
    if (f.is_open())
    {
        f << (int)iB << '\t' << k_max << '\t' << global_config.width_problem << '\t' << (int) ws.nBuckets() << '\t' << (int) ws.count_ComputeOutputFunction_NN << '\t' << global_config.avg_val_mse << '\t' << global_config.avg_test_mse << '\t' << global_config.avg_val_w_mse << '\t' << global_config.avg_test_w_mse << '\t'  <<global_config.avg_samples_req << '\t' << BEvalue_w << '\t' << (float) tElapsed/3600 << std::endl ;
        f.close();
    }
